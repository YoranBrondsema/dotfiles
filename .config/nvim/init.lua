	...
	--   -- you can continue same window with `<space>sr` which resumes last telescope search
	{
		...
		config = function()
			vim.g.copilot_workspace_folders = {
				"/Users/yoranbrondsema/Projects/Curvo/curvo",
			}
		end,
	},
	{
...
end, { desc = "[O]pen in current buffer" })
vim.keymap.set("n", "<leader>oh", function()
	vim.cmd("split")
	vim.lsp.buf.definition()
end, { desc = "[O]pen in [H]orizontal split" })
vim.keymap.set("n", "<leader>ov", function()
	vim.cmd("vsplit")
	vim.lsp.buf.definition()
end, { desc = "[O]pen in [V]ertical split" })

require("overseer").setup()

-- For `d` (delete) use `m` (for [M]ove) instead (see
-- https://github.com/svermeulen/vim-cutlass?tab=readme-ov-file#cutlassvim)
